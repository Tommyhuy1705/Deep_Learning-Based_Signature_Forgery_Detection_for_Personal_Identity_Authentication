...
def train_model_kfold(config, loss_fn:TripletLoss, dataset, k_folds:int, batch_size:int=32):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        train_loader = DataLoader(train_subset, num_workers=8, batch_size=batch_size, shuffle=True)
        val_loader = DataLoader(val_subset, num_workers=8, batch_size=batch_size, shuffle=False)
